...
type Service struct {
	...
	// Verifies authentication based on selected method (atomic for hot reload)
...
func NewService(cfg *v1.ServerConfig) (*Service, error) {
	...
	// Initialize atomic auth verifier from the auth runtime
	svr.authVerifier.Store(authRuntime.Verifier)
...
func (svr *Service) RegisterControl(ctlConn net.Conn, loginMsg *msg.Login, internal bool) error {
	...
	ctl, err := NewControl(ctx, svr.rc, svr.pxyManager, svr.pluginManager, svr, svr.auth.EncryptionKey(), ctlConn, !internal, loginMsg, svr.cfg)
	if err != nil {
